...
/usr/bin/ld: cannot find -laio collect2:
error: ld returned 1 exit status
make: *** [Makefile:526: blib/arch/auto/DBD/Oracle/Oracle.so] Error 1 ZARQUON/DBD-Oracle-1.95.tar.gz /usr/bin/make -- NOT OK
```
...
This is coming from Oracle client detection, not CPAN. To fix the issue, follow the steps:

``` 
cpan > look DBD::Oracle
```

Above command will provide the build directory (i.e: /home/nagios/.cpan/build/DBD-Oracle-1.95-4).
	
In bash execute the following
...
Then regenerate Makefile with clean LIBS by running the commands in following sequence
...
4.  Build & compile: check_oracle_health-3.3.3.2 downloaded from https://omd.consol.de/docs/plugins/check_oracle_health
   Execute
	...
	```
 	The last above command will generate the "configure" file
    		
	For some reason, I run in an small issue with, but if you don't face any issue, skipp this part and go to next instruction:
	Error:
...
6.	To add configurations for oracle db monitoring, check the official documentation, but what I did is:
...
7.	Edit the configuration file: ``` /usr/local/nagios/etc/nagios.cfg ```

    Add one line: ``` cfg_dir=/usr/local/nagios/etc/hosts ``` where <hosts> is a custom directory I created to organize the hosts.
...
Note: For service_name, I have already exported ``` TNS_ADMIN=$ORACLE_HOME/network/admin ``` and created a ``` tnsnames.ora ``` with the entries on it (tnsping succeeded).
