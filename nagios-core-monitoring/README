...
I used OS user "nagios" for all the configurations:
...
5. Run: sudo systemctl edit nagios and paste:
...
7. Edit the configuration file: /usr/local/nagios/etc/nagios.cfg
    ...
    Add line: cfg_dir=/usr/local/nagios/etc/hosts where "hosts" is a custom directory 
...
8. Copy or create a config file for each oracle server you want to monitor:
    ...
    Edit /usr/local/nagios/etc/hosts/<oracle-server>.cfg by changing: host_name <oracle-server>.
